...
extension XCTestCase {
    ...
    func withLocalNode(lifecycle: Bool = true, prefix: Bool = false, wait: Bool = false, test: (LocalNode, TestState, DispatchGroup) -> ()) {
        let node = TestNode()
...
class StructuredServiceTests: XCTestCase {
    let bag = DisposeBag()
    ...
    static var allTests: [(String, (StructuredServiceTests) -> () throws -> Void)] {
        return [
